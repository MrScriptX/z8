...
layout(std430, binding = 3) buffer WaterDraw {
    uint index_count;
    uint instance_count;
    uint first_index;
    int  vertex_offset;
...
// gl_VertexIndex = index_buffer[first_index + i] + vertex_offset;
layout(std430, binding = 4) buffer ChunkData {
    uint active_count;
...
void main()
{
    if (gl_GlobalInvocationID == uvec3(0)) {
        solid_cmd.index_count = 0;
        solid_cmd.instance_count = 1;
        solid_cmd.first_index = 0;
        solid_cmd.vertex_offset = 0;
        solid_cmd.first_instance = 0;

        water_cmd.index_count = 0;
        water_cmd.instance_count = 1;
        water_cmd.first_index = int(solid_voxel_count * 6);
        water_cmd.vertex_offset = int(solid_voxel_count * 4);
...
void create_quad(vec3 pos, uint dir, uvec2 size, vec3 normal, bool water, uint type)
{
    ...
    const uint vertex_base = type == WATER ? atomicAdd(water_quad_count, 4) : atomicAdd(active_count, 4);
    const uint vertex_offset = type == WATER ? water_cmd.vertex_offset : 0;
    vertices[vertex_offset + vertex_base + 0] = v[0];
    vertices[vertex_offset + vertex_base + 1] = v[1];
    vertices[vertex_offset + vertex_base + 2] = v[2];
    vertices[vertex_offset + vertex_base + 3] = v[3];

    const uint index_base = type == WATER ? atomicAdd(water_cmd.index_count, 6) : atomicAdd(solid_cmd.index_count, 6);
    const uint index_offset = type == WATER ? water_cmd.first_index : 0;
    indices[index_offset + index_base + 0] = vertex_base + 0;
    indices[index_offset + index_base + 1] = vertex_base + 1;
    indices[index_offset + index_base + 2] = vertex_base + 2;
    indices[index_offset + index_base + 3] = vertex_base + 0;
    indices[index_offset + index_base + 4] = vertex_base + 2;
    indices[index_offset + index_base + 5] = vertex_base + 3;
}
