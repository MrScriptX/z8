...
void main() {
    ...
    if (Chunk.voxels[index].data.x == AIR || Chunk.voxels[index].data.x == WATER) { // is not active
        return;
    }

    // encode faces in y
    const uint x_plus = (x + 1) + (y * CHUNK_SIZE) + (z * CHUNK_SIZE_SQR);
    if (x < CHUNK_SIZE - 1 && Chunk.voxels[x_plus].data.x != AIR && Chunk.voxels[x_plus].data.x != WATER) {
        Chunk.voxels[index].data.y |= FACE_POS_X; // +X
    }

    const uint x_minus = (x - 1) + (y * CHUNK_SIZE) + (z * CHUNK_SIZE_SQR);
    if (x > 0 && Chunk.voxels[x_minus].data.x != AIR && Chunk.voxels[x_minus].data.x != WATER) {
        Chunk.voxels[index].data.y |= FACE_NEG_X; // -X
    }

    const uint y_plus = x + ((y + 1) * CHUNK_SIZE) + (z * CHUNK_SIZE_SQR);
    if (y < CHUNK_SIZE - 1 && Chunk.voxels[y_plus].data.x != AIR && Chunk.voxels[y_plus].data.x != WATER) {
        Chunk.voxels[index].data.y |= FACE_POS_Y; // +Y
    }

    const uint y_minus = x + ((y - 1) * CHUNK_SIZE) + (z * CHUNK_SIZE_SQR);
    if (y > 0 && Chunk.voxels[y_minus].data.x != AIR && Chunk.voxels[y_minus].data.x != WATER) {
        Chunk.voxels[index].data.y |= FACE_NEG_Y; // -Y
    }

    const uint z_plus = x + (y * CHUNK_SIZE) + ((z + 1) * CHUNK_SIZE_SQR);
    if (z < CHUNK_SIZE - 1 && Chunk.voxels[z_plus].data.x != AIR && Chunk.voxels[z_plus].data.x != WATER) {
        Chunk.voxels[index].data.y |= FACE_POS_Z; // +Z
    }

    const uint z_minus = x + (y * CHUNK_SIZE) + ((z - 1) * CHUNK_SIZE_SQR);
    if (z > 0 && Chunk.voxels[z_minus].data.x != AIR && Chunk.voxels[z_minus].data.x != WATER) {
